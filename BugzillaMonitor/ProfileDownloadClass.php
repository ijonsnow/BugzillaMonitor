...
class ProfileDownload
{
	...
	private function ProfileTimerQuery()
     {
		return mysql_query("SELECT t2.realname,t1.who,t1.profiles_when,t1.fieldid,t1.oldvalue,t1.newvalue FROM profiles_activity AS t1 Inner JOIN profiles AS t2 ON t1.userid = t2.userid and t1.profiles_when >= '$this->Dte' ORDER BY t1.profiles_when DESC");	  
     }  
 
    private function ProfileHistoryQuery()
     {
		return mysql_query("SELECT t2.realname,t1.who,t1.profiles_when,t1.fieldid,t1.oldvalue,t1.newvalue FROM profiles_activity AS t1 Inner JOIN profiles AS t2 ON t1.userid = t2.userid and t1.profiles_when Between '$this->FromDate' and '$this->ToDate' ORDER BY t1.profiles_when DESC");	  
     }
	...
	{		 
		...
		//  Gets the data from the database
			$this->ExportFile='ProfileHistory.csv';
			$result = $this->ProfileHistoryQuery();
			$fields_cnt = mysql_num_fields($result);
		}
		else
		{
		//  Gets the data from the database
			$this->ExportFile='ProfileTimer.csv';
			$result = $this->ProfileTimerQuery();
			$fields_cnt = mysql_num_fields($result);
		}	
